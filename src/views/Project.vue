  ...
  <div class="mx-0 px-0">
    <v-row style="width: 100%;overflow-x:auto; white-space: nowrap;" >
      <v-col class="px-0" style="position: relative;" sm="6" md="3" xs="12" lg="2" cols="12"
                ...
                <template v-slot:item="{item}">
                  <drag :key="item.id" :data="item" @cut="removeItem(status.tasks, item)">
                    <v-sheet class="pointer v-sheet--outlined mx-0 my-0">
                      <v-alert @click.prevent="getTaskDetails(item)" class="ma-0" v-if="item.completed" text dense
                               type="success">Tapşırıq bitdi
                      </v-alert>
                      <div @click.prevent="getTaskDetails(item)"
                           class="px-4 py-3 black--text elevation-2 font-weight-medium subtitle-2">
                        <span class="d-block"
                              :class="{'text-decoration-line-through': status.name.toLowerCase() == 'done' || item.completed}">{{
                            item.name
                          }}</span>
        ...
        <div v-show="index === (project.statuses.length - 1) && !showStatusCard" class="rounded-circle addSection">
          <v-btn @click.prevent="showStatusCard = !showStatusCard" small rounded dark icon>
            <v-icon>mdi-plus</v-icon>
          </v-btn>
        </div>
      </v-col>
      <v-col class="px-0" v-if="showStatusCard" sm="6" xs="12" lg="2" cols="12">
        <v-card height="100vh">
        ...
        <div v-show="showStatusCard" class="rounded-circle addSection">
          <v-btn @click.prevent="showStatusCard = true" small rounded dark icon>
            <v-icon>mdi-plus</v-icon>
          </v-btn>
        </div>
      </v-col>
                  ...
                  <v-col cols="12">
                    <v-sheet height="80" width="100%" class="d-flex flex-row flex-shrink-1">
                      <v-flex shrink="1" grow="0" class="d-flex justify-space-around flex-row align-start">
                        <v-btn :disabled="taskSlot.task.completed === 1 ? 'disabled' : null" class="flex-shrink-1 mr-5"
                               height="48" @click.prevent="taskSlot.task.completed = 1; taskUpdate(taskSlot.task)"
                               color="primary"> Bİtİr
                        </v-btn>
                        <v-spacer></v-spacer>
                        <v-select v-model.trim="assign" @change="assignTaskToEmployee" :items="department.employees"
                                  item-value="id" item-text="name" clearable class="flex-grow-0" solo
                      ...
                      <v-flex class="d-flex flex-shrink-1 align-start justify-end">
                        ...
                        >
                          ...
                          <v-list>
                            ...
                            <v-list-item
                                v-show="taskSlot.task.completed === 1"
                                link
                                @click.prevent="restoreTask(taskSlot.task)"
                            >
                              <v-list-item-title>Aktiv Et</v-list-item-title>
                            </v-list-item>
                          </v-list>
                        </v-menu>
                      </v-flex>
                  ...
                  <div class="col-4 pl-0 ml-0 mr-0 pr-0">
                    <v-col cols="12" class="pl-0 pr-0">
                      <v-card class="mt-3 pl-0">
                        <v-menu bottom>
                          ...
                          <v-date-picker @change="updateTask(taskSlot.task)" v-model.trim="taskSlot.task.deadline"
                                         flat></v-date-picker>
                        </v-menu>
                      </v-card>
                    </v-col>
                    <v-col cols="12" class="pl-0 pr-0">
                      <v-card color="#ddd" class="mt-3 pl-0">
                        <v-card-text>
                          <h2 class="subtitle-1">
                            <v-icon class="mr-2" small>mdi-shape-square-rounded-plus</v-icon>
                            Təyin olundu
                          </h2>
                          <p class="body-2 ml-9"
                             v-text="taskSlot.task.created_at !== null ? taskSlot.task.created_at.substring(0,10) : null"></p>
                          ...
                          <p class="body-2 ml-9"
                             v-text="taskSlot.task.updated_at !== null ? taskSlot.task.updated_at.substring(0,10) : null"></p>

                          <h2 class="subtitle-1">
                            <v-icon class="mr-2" small>mdi-link-variant</v-icon>
                            Tapşırıq Nömrəsi
                          </h2>
  ...
  methods: {
    ...
    getTaskDetails(task) {
      ...
      this.getCommentsFromApi(task.id).then(() => {
        this.assign = this.department.employees.filter(emp => {
          return emp.id === this.task.assigned_to ? emp.id : null
        })
    ...
    addSectionToApi() {
      ...
      console.log(this.colors[this.takeRandomColor()])
        ...
        this.addSection = true
        this.storeStatus({
          name: vm.section_name,
          color: this.colors[this.takeRandomColor()],
